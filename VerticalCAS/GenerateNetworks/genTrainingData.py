...
# OPTIONS #######
tableFile = Path(Path(__file__).parent.parent, "GenerateTable", "VCAS.h5")
trainingDataFiles = str(Path(__file__).parent) + "/VCAS_TrainingData_v5_%02d.h5"
######################

# Define state space. Make sure this matches up with the constants used to generate the MDP table!
acts = [0, 1, 2, 3, 4, 5, 6, 7, 8]
vels = np.concatenate((
    np.linspace(-100, -60, 5),
    np.linspace(-50, -35, 4),
...
hs = np.concatenate((
    np.linspace(-8000, -4000, 5),
    np.linspace(-3000, -1250, 8),
    np.linspace(-1000, -800, 3),
    np.linspace(-700, -150, 12),
    np.linspace(-100, 100, 9),
    np.linspace(150, 700, 12),
    np.linspace(800, 1000, 3),
    np.linspace(1250, 3000, 8),
    np.linspace(4000, 8000, 5),
))
vowns = vels
vints = vels
...
f = h5py.File(str(tableFile.resolve()), "r")
Q = np.array(f["q"])
f.close()
Q = Q.T
...
Qacts = [
    np.concatenate([Qtaus[i][ns * j : (j + 1) * ns] for i in range(len(taus))])
...
rangeQ = np.max([np.max(Qacts[i]) for i in range(len(Qacts))]) - np.min([
    np.min(Qacts[i]) for i in range(len(Qacts))
])
